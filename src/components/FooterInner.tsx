...
export default function FooterInner({ site, friendlyLinks, socialLinks }: FooterInnerProps) {
  ...
  // Get platform name from translation
  const getPlatformName = (platform: string): string => {
    const key = `footer.socialPlatforms.${platform}`;
    const translated = t(key);
    // If translation returns the key itself (not found), fall back to Chinese default
    if (translated === key) {
